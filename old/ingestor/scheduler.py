...
async def loop(cfg: Config, client: GammaClient):
    ...
    try:
        ...
        while True:
            ...
            try:
                await db.archive_and_delete_minutes(
                    db_conn, 
                    table_name=db.TBL_M_1M,
                    retention_period=timedelta(minutes=60)
                )
                # You can add more lines here for other tables, e.g., 1H data
                # await db.archive_and_delete_minutes(
                #     db_conn,
                #     table_name=db.TBL_M_1H,
                #     retention_period=timedelta(days=7)
                # )
            except Exception as e:
                print(f"Archive run failed: {e}")
